...
Nomos is a Go codebase for a policy-enforced execution control plane. The CLI entrypoint lives in `cmd/nomos`. Core implementation packages live under `internal/` and include areas such as `policy`, `service`, `gateway`, `mcp`, `doctor`, `executor`, and `agenthook` (the Claude Code and Codex hooks). Example configs, quickstarts, and incident regression suites live in `examples/`. Starter policy bundles are in `examples/policies/`, the default profiles in `profiles/` (embedded copies are generated by `make pin-profile-hashes`), docs in `docs/`, and reusable fixtures in `testdata/`.
...
- `go test ./internal/agenthook -run Corpus` checks the real-world corpus golden; after an intended profile change, regenerate it with `-update-corpus` and review the diff.
